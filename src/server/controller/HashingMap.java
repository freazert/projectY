...
public class HashingMap
{
	...
	/*
	 * public HashingMap(Hashing hash) { this.hashMap = new HashMap<Integer,
	 * String>(); addRecord(hash, "192.168.1.1"); }
	 */
	 ...
	 * 
	 * @param hash
	 *            The hash that needs to be inserted in the hashingmap
	 * @param ip
	 ...
	 * 
	 * @param filename
	 *            The name of the file that needs to be found.
	...
	/*
	 * public String getPrevIp(String filename) { Map.Entry<Integer, String>
	 * record = null; Map.Entry<Integer, String> highestRecord = null;
	 * Map.Entry<Integer, String> lowestRecord = null; int fileHash = new
	 * Hashing(filename).getHash();
	 * 
	 * Iterator it = this.hashMap.entrySet().iterator();
	 * System.out.println("searching... filehash = " + fileHash); while
	 * (it.hasNext()) { Map.Entry<Integer, String> pair = (Map.Entry)it.next();
	 * System.out.println("pair key = " + pair.getKey()); if( record == null) {
	 * record = pair; System.out.println("new at start: " + record.getKey());
	 * highestRecord = pair; lowestRecord = pair; } else { //set lowest and
	 * highest hashes if(highestRecord.getKey() < pair.getKey()) { highestRecord
	 * = pair; } else if(lowestRecord.getKey() > pair.getKey()) { lowestRecord =
	 * pair; }
	 * 
	 * //check for new record if(fileHash > pair.getKey() && (pair.getKey() >
	 * record.getKey() || fileHash < record.getKey())){ record = pair;
	 * System.out.println("new: " + record.getKey());
	 * //System.out.Println(record.getKey().toString()); } } }
	 * 
	 * 
	 * // if fileHash is higher than highest hash, take lowest hash if(fileHash
	 * < lowestRecord.getKey()) { record = highestRecord;
	 * System.out.println("new at end: " + record.getKey()); }
	 * System.out.println("Search complete"); System.out.println("result: " +
	 * record.getKey());
	 * 
	 * return record.getValue(); }
	 */
